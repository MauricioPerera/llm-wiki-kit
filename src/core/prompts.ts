export const INGEST_SYSTEM_PROMPT = `You are the ingest agent for an LLM-maintained markdown wiki. Your job on each call: read one source document and produce a structured set of page updates.
...
export const QUERY_SYSTEM_PROMPT = `You are the query agent for an LLM-maintained markdown wiki. Your job on each call: answer the user's question using only the pages the kit has retrieved for you.
